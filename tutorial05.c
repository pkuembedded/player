...
double get_audio_clock(VideoState *is) {
  ...
  bytes_per_sec = 0;
  //fprintf(stderr,"The channels of the audio is %d/n",is->audio_st->codec->channels);
  n = 4;
...
int queue_picture(VideoState *is, AVFrame *pFrame, double pts) {
  ...
  if(vp->bmp) {
    ...
    /*img_convert(&pict, dst_pix_fmt,
		...
		is->video_st->codec->width, is->video_st->codec->height);*/
     sws_scale(is->img_convert_ctx, (const uint8_t* const*)pFrame->data, pFrame->linesize, 					0,is->video_st->codec->height, pict.data, pict.linesize);    
...
int decode_thread(void *arg) {
  ...
  for(i=0; i<pFormatCtx->nb_streams; i++) {
    if(pFormatCtx->streams[i]->codec->codec_type==AVMEDIA_TYPE_VIDEO) {
      video_index=i;
      fprintf(stderr,"%s:find a video stream\n",is->filename);
      break;
    }

    if(pFormatCtx->streams[i]->codec->codec_type==AVMEDIA_TYPE_AUDIO) {
      fprintf(stderr,"%s:find a audio stream\n",is->filename);
      audio_index=i;
      break;
    }
  }
